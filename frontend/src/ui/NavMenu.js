...
function ResponsiveAppBar() {
  const [pages, setPages] = useState([
    ...
    { name: "how it works", link: "/more", current: false },
  ]);
  ...
  const handleCloseNavMenu = (index) => {
    setAnchorElNav(null);
    const updatedItems = pages.map((item, i) =>
          ...
          <Box sx={{ flexGrow: 1, display: { xs: "none", md: "flex" } }}>
            {pages.map((page, index) => (
              <Button
                ...
                className={`rounded-md me-4 text-navy-blue hover-underline-animation ${
                  page.current ? "text-bright-yellow" : ""
                }`}
